...
**高品質朗讀：** 🔊 預設用系統內建語音（離線、零設定）。想要更自然的神經語音，在 ⚙ 把「朗讀引擎」選 **高品質本機（Piper）** —— 神經語音引擎（[sherpa-onnx](https://github.com/k2-fsa/sherpa-onnx)，Apache-2.0）已**內建在 app 裡**，第一次按 🔊 會自動下載語音模型（[Piper](https://github.com/rhasspy/piper) `en_US-amy-medium`，~64MB，之後快取），**全離線、本機合成、不需自己架 server**。
