...
- **朗讀整句** —— 譯文結尾的 🔊 一鍵朗讀整句英文（再按一次停止）。預設用系統內建語音（離線、免費）；想要最自然音色可在 ⚙ 開「高品質朗讀」接 OpenAI 相容 TTS 端點（如本機 **Kokoro-FastAPI**）。
...
**高品質朗讀（Kokoro）：** 預設 🔊 用系統內建語音（離線、免費）。想要最自然的開源音色，在 ⚙ 勾「高品質朗讀」，並在本機跑一個 OpenAI 相容的 TTS 服務 —— 推薦 [**Kokoro-FastAPI**](https://github.com/remsky/Kokoro-FastAPI)（Kokoro-82M，Apache-2.0）：

```bash
docker run -p 8880:8880 ghcr.io/remsky/kokoro-fastapi-cpu   # 或 -gpu 版
```

起來後 ⚙ 的「TTS 端點」留預設 `http://localhost:8880`（會自動補成 `/v1/audio/speech`）、語音填 `af_heart`（其他如 `af_bella`/`am_adam`/`bf_emma`…）。任何 OpenAI 相容 TTS（含雲端 `tts-1`）都能接；端點連不上會自動退回系統語音。
